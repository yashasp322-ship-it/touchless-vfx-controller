...
function startRecording() {
    ...
    mediaRecorder.onstop = () => {
        ...
        a.href = url;
        a.download = `VFX_Magic_${Date.now()}.webm`;
        a.click();
...
function setPower(power) {
    ...
    // Collapse the surrounding box when a feature is selected
    uiPanel.classList.add('minimized');
}

// UI Draggable functionality
...
dragHandle.addEventListener('mousedown', (e) => {
    ...
    // Remove "centered" classes so we can freely move it with absolute values
    uiPanel.style.transform = 'none';
    uiPanel.style.bottom = 'auto';
    uiPanel.style.left = `${rect.left}px`;
    uiPanel.style.top = `${rect.top}px`;
    uiPanel.style.transition = 'none'; // Disable transition so dragging is smooth
});
...
document.addEventListener('mousemove', (e) => {
    ...
    // Auto-detect left or right edges to switch to vertical mode
    const screenWidth = window.innerWidth;
    // If it is in the leftmost 20% or rightmost 20%
    if (newX < screenWidth * 0.2 || (newX + uiPanel.offsetWidth) > screenWidth * 0.8) {
        uiPanel.classList.add('vertical');
    } else {
        uiPanel.classList.remove('vertical');
    }
});
...
document.addEventListener('mouseup', () => {
    if (isDragging) {
        isDragging = false;
        uiPanel.style.transition = 'all 0.3s ease'; // Turn transition back on for scale animations
    }
});
...
function onResults(results) {
    // Update Connection Status if results are flowing
    if (results) {
        if (!statusDot.classList.contains('active')) {
            statusDot.classList.add('active');
            statusText.textContent = 'Camera & AI Active';
        }
    }
    ...
    // Draw loading overlay if still waiting for hands detection
    if (!aiInitialized) {
        aiInitialized = true;
        statusText.textContent = 'AI Initialized and Ready';
    }
...
const camera = new Camera(videoElement, {
    onFrame: async () => {
        if (!aiInitialized) {
            // Draw video to canvas directly to prove camera is working while models load
            canvasElement.width = window.innerWidth;
            canvasElement.height = window.innerHeight;
            canvasCtx.save();
            canvasCtx.clearRect(0, 0, canvasElement.width, canvasElement.height);
            ...
            canvasCtx.font = '24px Outfit, sans-serif';
            canvasCtx.textAlign = 'center';
            canvasCtx.fillText("Downloading AI Models... (this can take a few seconds on first run)", canvasElement.width/2, canvasElement.height/2);
            canvasCtx.restore();
        }
...
camera.start().then(() => {
    statusText.textContent = "Camera Connected. Loading AI Processors...";
}).catch((err) => {
